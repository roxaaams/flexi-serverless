...
std::string exec(const char* command) {
 ...
 std::string invokeAWS(const std::string& function_name, const std::string& payload) {
    std::string command = "aws lambda invoke --payload '" + payload + "' --function-name " + function_name + " --cli-binary-format raw-in-base64-out responses/response.json >nul 2>nul";
//    std::cout << command << std::endl;
    system(command.c_str());
    std::string result = exec("cat responses/response.json");
    return result;
}
...
//    std::cout << command << std::endl;
 ...
 std::string invoke(const std::string& function_name, const std::string& payload) {
    std::string provider = getENV();
    if (provider == "AWS") {
        return invokeAWS(function_name, payload);
